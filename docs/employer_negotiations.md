...
* [Выдача предпочитаемой сортировки откликов] (#get-preference-sort)
...
Для **создания приглашения** необходимо выбрать резюме, например,
[в поиске](employer_resumes.md#search), а затем запросить вакансии работодателя,
применимые к этому резюме. Получить эту информацию можно в
...
resume | объект, null | Резюме. Выдаваемые поля аналогичны [поиску резюме](employer_resumes.md#search-results). Для получения полного резюме необходимо запросить его GET запросом по url из ключа `url`. Может быть `null`, если соискатель удалил резюме или закрыл к нему доступ.
...
## Выдача предпочитаемой сортировки откликов

### Запрос

`GET /negotiations/preference_sort`
